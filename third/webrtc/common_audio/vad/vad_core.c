...
//static const int16_t kNoiseDataMeansKST[kTableSize] = {
//    4554, 4097, 3739, 2945, 2751, 2809, 5009, 4189, 3733, 3089, 2791, 2930 };
//// Means for the two Gaussians for the six channels (speech)
//static const int16_t kSpeechDataMeansKST[kTableSize] = {
//    7685, 11176, 7715, 12160, 12160, 5539, 11830, 8413, 11373, 6407, 4968, 10676};
//// Stds for the two Gaussians for the six channels (noise)
//static const int16_t kNoiseDataStdsKST[kTableSize] = {
//    395, 1070, 515, 590, 698, 614, 480, 703, 418, 695, 428, 461 };
//// Stds for the two Gaussians for the six channels (speech)
//static const int16_t kSpeechDataStdsKST[kTableSize] = {
//    1127, 568, 1175, 531, 592, 1652, 515, 1201, 577, 1625, 1754, 856 };

//static const int16_t kNoiseDataMeansGen[kTableSize] = {
//    4510, 4100, 3700, 2950, 2755, 2798, 4996, 4200, 3820, 3100, 2820, 3162 };
//// Means for the two Gaussians for the six channels (speech)
//static const int16_t kSpeechDataMeansGen[kTableSize] = {
//    8306, 10085, 8003, 11823, 11843, 5600, 11800, 8300, 10879, 6300, 5002, 9862};
//// Stds for the two Gaussians for the six channels (noise)
//static const int16_t kNoiseDataStdsGen[kTableSize] = {
//    378, 1064, 493, 582, 688, 593, 474, 697, 475, 688, 421, 455};
//// Stds for the two Gaussians for the six channels (speech)
//static const int16_t kSpeechDataStdsGen[kTableSize] = {
//1002, 505, 993, 524, 585, 1331, 509, 1199, 502, 1540, 1579, 850};
...
//mode 4 Kst
static const int16_t kOverHangMax1Kst[3] = { 7, 4, 2 };
static const int16_t kOverHangMax2Kst[3] = { 9, 5, 3 };
static const int16_t kLocalThresholdKst[3] = { 90, 89, 90 };
static const int16_t kGlobalThresholdKst[3] = { 1000, 980, 1000 };
...
int WebRtcVad_InitCore(VadInstT* self,
                       int16_t noise_data_means[kTableSize],
                       int16_t speech_data_means[kTableSize],
                       int16_t noise_data_stds[kTableSize],
                       int16_t speech_data_stds[kTableSize]) {
    int i;
    ...
    for (i = 0; i < kTableSize; i++) {
        self->noise_means[i] = noise_data_means[i];
        self->speech_means[i] = speech_data_means[i];
        self->noise_stds[i] = noise_data_stds[i];
        self->speech_stds[i] = speech_data_stds[i];
    }
...
int WebRtcVad_set_mode_core(VadInstT* self, int mode) {
    ...
    case 4:
        // Kst model
        memcpy(self->over_hang_max_1, kOverHangMax1Kst,
               sizeof(self->over_hang_max_1));
        memcpy(self->over_hang_max_2, kOverHangMax2Kst,
               sizeof(self->over_hang_max_2));
        memcpy(self->individual, kLocalThresholdKst,
               sizeof(self->individual));
        memcpy(self->total, kGlobalThresholdKst,
               sizeof(self->total));
        break;
    default:
        return_value = -1;
