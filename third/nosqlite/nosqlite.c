 ...
 */
static void _reverse_data(unsigned char *p, int len)
{
	unsigned char *e;
    ...
    if (!db->file) {
        ...
        if (size != 12 || strncmp(version, NOSQLITE_VERSION, 12)) { /* verify version */
            ...
            while (1) {
				...
				_reverse_data((unsigned char*)key, klen);
...
nosqlite_set(struct nosqlite *db, const void *key, int _klen, const void *value, int _vlen)
{
	...
	_reverse_data((unsigned char*)value, vlen);
...
char* nosqlite_version(struct nosqlite *db)
{
	char* v = NOSQLITE_VERSION;
