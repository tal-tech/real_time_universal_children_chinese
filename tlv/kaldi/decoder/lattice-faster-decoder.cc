...
BaseFloat LatticeFasterDecoderTpl<FST, Token>::ProcessEmitting(
  ...
  // focus words
  bool has_hot_words = false;
  if(hot_words_.size() > 0)
  {
      has_hot_words = true;
  }
    ...
    if (tok->tot_cost <= cur_cutoff) {
      for (fst::ArcIterator<FST> aiter(*fst_, state);
        ...
        if (arc.ilabel != 0) {  // propagate..
          ...
          if(has_hot_words)
          {
              std:: vector<int64>::iterator iter_find;
              iter_find = std::find(hot_words_.begin(),hot_words_.end(),arc.olabel);
              if(iter_find != hot_words_.end())
              {
                  graph_cost -= det_graph_cost_;
                  tot_cost = cur_cost + ac_cost + graph_cost;
              }
          }
