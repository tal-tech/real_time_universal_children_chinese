...
class LatticeFasterDecoderTpl {
 public:
  ...
  /// call SetExpectWords before call Decode() every time
  /// if set expect words,the expect words will be take a lower graph_cost=graph_cost-det_graph_cost
  inline void SetExpectWords(const std::vector<int64>& words, BaseFloat det_graph_cost){ hot_words_ = words; det_graph_cost_ = det_graph_cost; }
 ...
 protected:
  ...
  /// focus words can set before decode
  /// if set focus words,the focus words will be take a lower graph cost
  std::vector<int64> hot_words_;
  BaseFloat det_graph_cost_;
